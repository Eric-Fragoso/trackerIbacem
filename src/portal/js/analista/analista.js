      ...
      document.getElementById('containerQualiadade').innerHTML = images.map(function (qualidade) {
        ...
        .catch(function(error){
          ...
          var aprovadoAtual = `<a href="javascript:;"><i class="fas fa-check-circle"><input type="checkbox" id=${qualidade._id} value="hidden" class="checkboxVisivel"></i></a>`;
            return (
              `<tr align="center"><td id="tdFornecedor${qualidade._id}"></td><td id="tdConrole${qualidade._id}"></td><td>${qualidade.etapaRelacionada}</td><td><a href="javascript:;" onclick="linkPdf('${qualidade.controleRelacionado}','${qualidade.etapaRelacionada}');" class="editarControleProdutor"><i class="fas fa-file-pdf"></i> Relatório</a></td><td></td><td><div class="tdClicavel" onclick="changeCheckboxStateQual('${qualidade._id}');">${aprovadoAtual}</div></td></tr>`
              );
            }else{
  ...
  await axios.get(`http://138.204.68.18:3324/api/relatorio/${fornecedorCod}/${controleCod}`)
        .then(function(resposta){
          ...
          controls.map(function(controle){
            ...
            <td>${(controle.FRETE_COLHEITA).toFixed(2)}</td>
            <td>${(controle.FUNRURAL).toFixed(2)}</td>
